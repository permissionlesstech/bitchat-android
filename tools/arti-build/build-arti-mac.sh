...
# Determine NDK host tag for macOS by probing what's actually installed
if [ "$(uname -s)" != "Darwin" ]; then
  echo -e "${RED}Error: This script currently supports macOS only (Darwin).${NC}"
  exit 1
fi

PREBUILT_DIR="$ANDROID_NDK_HOME/toolchains/llvm/prebuilt"

if [ -d "$PREBUILT_DIR/darwin-arm64" ]; then
  NDK_HOST="darwin-arm64"
elif [ -d "$PREBUILT_DIR/darwin-x86_64" ]; then
  NDK_HOST="darwin-x86_64"
else
  echo -e "${RED}Error: No macOS NDK prebuilt toolchain found under:${NC} $PREBUILT_DIR"
  echo -e "${YELLOW}Found:${NC}"
  ls -la "$PREBUILT_DIR" || true
  exit 1
fi

NDK_LLVM_BIN="$PREBUILT_DIR/$NDK_HOST/bin"
...
check_prerequisites() {
  ...
  if [ "${BASH_VERSINFO:-0}" -lt 4 ]; then
    ...
    print_info "Install: brew install bash"
    print_info "Then run with: /opt/homebrew/bin/bash ./build-arti.sh (Apple Silicon)"
    print_info "Or: /usr/local/bin/bash ./build-arti.sh (Intel)"
    exit 1
  fi
  ...
  if [ -x "$LLVM_NM" ]; then
    ...
    print_info "llvm-nm not found, will fall back to nm (may be less reliable for ELF on macOS)"
  fi
