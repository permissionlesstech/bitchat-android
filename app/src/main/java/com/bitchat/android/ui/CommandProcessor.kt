...
class CommandProcessor(
    ...
    private fun handleActionCommand(
        ...
        if (parts.size > 1) {
            val targetName = parts[1].removePrefix("@")
            val actionMessage =
                "* ${state.getNicknameValue() ?: "someone"} $verb $targetName $object_ *"
    ...
    fun updateMentionSuggestions(
        input: String, meshService: BluetoothMeshService,
        pair: Pair<List<GeoPerson>?, ChannelID?>? = null
    ) {
        ...
        val peerCandidates: List<String> = if (pair != null) {
            when (val selectedChannel = pair.second) {
                is com.bitchat.android.geohash.ChannelID.Mesh,
                null -> {
                ...
                is com.bitchat.android.geohash.ChannelID.Location -> {
                    ...
                    val geohashPeople = pair.first ?: emptyList()
                    val currentNickname = state.getNicknameValue()
