...
class BluetoothMeshService(private val context: Context) {
    ...
    private fun setupDelegates() {
        // Provide nickname resolver to connection managers for detailed logs
        try {
            connectionManager.setNicknameResolver { pid -> peerManager.getPeerNickname(pid) }
            wifiDirectManager.setNicknameResolver { pid -> peerManager.getPeerNickname(pid) }
        } catch (_: Exception) { }
        ...
        // WiFiDirectConnectionManager delegates
        wifiDirectManager.delegate = object : WiFiDirectConnectionManagerDelegate {
            override fun onPacketReceived(packet: BitchatPacket, peerID: String, device: android.net.wifi.p2p.WifiP2pDevice?) {
                packetProcessor.processPacket(RoutedPacket(packet, peerID, device?.deviceAddress))
            }

            override fun onDeviceConnected(device: android.net.wifi.p2p.WifiP2pDevice) {
                // Send initial announcements after services are ready
                serviceScope.launch {
                    delay(200)
                    sendBroadcastAnnounce()
                }
                // Log Wi-Fi Direct connection
                try {
                    val addr = device.deviceAddress
                    val peer = wifiDirectManager.getDeviceAddressForPeer("") // We need to map this
                    val nick = peer?.let { peerManager.getPeerNickname(it) } ?: device.deviceName ?: "unknown"
                    com.bitchat.android.ui.debug.DebugSettingsManager.getInstance()
                        .logPeerConnection(peer ?: "unknown", nick, addr, isInbound = true)
                } catch (_: Exception) { }
            }

            override fun onDeviceDisconnected(device: android.net.wifi.p2p.WifiP2pDevice) {
                val addr = device.deviceAddress
                // Remove mapping and clear direct flag
                val peer = wifiDirectManager.getDeviceAddressForPeer("") // We need to map this
                if (peer != null) {
                    // Peer might still be reachable indirectly; mark as not-direct
                    try { peerManager.setDirectConnection(peer, false) } catch (_: Exception) { }
        ...
        // LocalNetworkConnectionManager delegates
        localNetworkManager.delegate = object : LocalNetworkConnectionManagerDelegate {
            override fun onPacketReceived(packet: BitchatPacket, peerID: String, address: String) {
                packetProcessor.processPacket(RoutedPacket(packet, peerID, address))
            }

            override fun onDeviceConnected(address: String, peerID: String) {
                // Send initial announcements after services are ready
                serviceScope.launch {
                    delay(200)
                    sendBroadcastAnnounce()
                }
                // Log local network connection
                try {
                    val nick = peerManager.getPeerNickname(peerID) ?: "unknown"
                    com.bitchat.android.ui.debug.DebugSettingsManager.getInstance()
                        .logPeerConnection(peerID, nick, address, isInbound = true)
                } catch (_: Exception) { }
            }

            override fun onDeviceDisconnected(address: String) {
                // Find peer ID for this address and mark as disconnected
                try {
                    // We need to maintain a mapping from address to peer ID
                    // For now, we'll just log the disconnection
                    com.bitchat.android.ui.debug.DebugSettingsManager.getInstance()
                        .logPeerDisconnection("unknown", "unknown", address)
                } catch (_: Exception) { }
            }
        }
    }
    ...
    fun startServices() {
        ...
        Log.i(TAG, "Starting mesh services with peer ID: $myPeerID")

        var bluetoothStarted = false
        var wifiDirectStarted = false

        // Try to start Bluetooth services
        if (connectionManager.startServices()) {
            bluetoothStarted = true
            Log.d(TAG, "Bluetooth services started successfully")
        } else {
            Log.w(TAG, "Failed to start Bluetooth services")
        }

        // Try to start Wi-Fi Direct services
        if (wifiDirectManager.startServices()) {
            wifiDirectStarted = true
            Log.d(TAG, "Wi-Fi Direct services started successfully")
        } else {
            Log.w(TAG, "Failed to start Wi-Fi Direct services")
        }

        // Try to start Local Network services
        var localNetworkStarted = false
        try {
            val debugManager = com.bitchat.android.ui.debug.DebugSettingsManager.getInstance()
            val localNetworkEnabled = debugManager.localNetworkEnabled.value
            if (localNetworkEnabled && localNetworkManager.startServices()) {
                localNetworkStarted = true
                Log.d(TAG, "Local Network services started successfully")
            } else if (localNetworkEnabled) {
                Log.w(TAG, "Failed to start Local Network services")
            }
        } catch (_: Exception) {
            Log.w(TAG, "Local Network not available")
        }

        // If at least one service started, consider mesh active
        if (bluetoothStarted || wifiDirectStarted || localNetworkStarted) {
            isActive = true
            ...
            Log.i(TAG, "Mesh services started: Bluetooth=$bluetoothStarted, WiFi Direct=$wifiDirectStarted")
        } else {
            Log.e(TAG, "Failed to start any mesh services")
        }
    }
    ...
    fun stopServices() {
        ...
        Log.i(TAG, "Stopping mesh services")
        isActive = false
        ...
        serviceScope.launch {
            ...
            wifiDirectManager.stopServices()
            localNetworkManager.stopServices()
    ...
    fun sendMessage(content: String, mentions: List<String> = emptyList(), channel: String? = null) {
        ...
        serviceScope.launch {
            ...
            // Sign the packet before broadcasting
            val signedPacket = signPacketBeforeBroadcast(packet)
            val routedPacket = RoutedPacket(signedPacket)
    ...
    fun sendFileBroadcast(file: com.bitchat.android.model.BitchatFilePacket) {
        ...
        serviceScope.launch {
            ...
            wifiDirectManager.broadcastPacket(RoutedPacket(signed, transferId = transferId))
            localNetworkManager.broadcastPacket(RoutedPacket(signed, transferId = transferId))
